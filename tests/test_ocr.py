...
@pytest.fixture()
def mock_ocr_client():
    """Provide a mocked httpx.AsyncClient for OCR tests."""
...
def test_parse_response_raises_on_missing_pages_key():
    data = {"error": "something went wrong"}
    with pytest.raises(OCRError, match="missing 'pages' key"):
        _parse_response(data)


def test_parse_response_raises_on_malformed_page_entry():
    data = {"pages": [{"text": "no page number"}]}
...
async def test_extract_text_returns_ocr_result(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    mock_ocr_client.post.return_value = _success_response()

    result = await extract_text(pdf, _make_config())
...
async def test_extract_text_raises_on_unsupported_file(tmp_path):
    ...
    with pytest.raises(OCRError, match="Unsupported file type"):
        await extract_text(docx, _make_config())
...
async def test_extract_text_raises_on_missing_file(tmp_path):
    ...
    with pytest.raises(OCRError, match="File not found"):
        await extract_text(missing, _make_config())


async def test_extract_text_retries_on_5xx_then_succeeds(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    ...
    mock_ocr_client.post.side_effect = [error_resp, _success_response()]

    with mock.patch("docproc.ocr.asyncio.sleep", new_callable=mock.AsyncMock):
        result = await extract_text(pdf, _make_config())

    assert result.text == "Hello world"
    assert mock_ocr_client.post.call_count == 2


async def test_extract_text_retries_on_timeout_then_succeeds(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    mock_ocr_client.post.side_effect = [
...
async def test_extract_text_retries_on_connect_error_then_succeeds(
    tmp_path, mock_ocr_client
):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    mock_ocr_client.post.side_effect = [
...
async def test_extract_text_raises_after_max_retries(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    ...
    mock_ocr_client.post.return_value = error_resp

    with (
        mock.patch("docproc.ocr.asyncio.sleep", new_callable=mock.AsyncMock),
        pytest.raises(OCRError, match="OCR failed after 3 attempts"),
    ):
        await extract_text(pdf, _make_config())

    assert mock_ocr_client.post.call_count == 3


async def test_extract_text_fails_immediately_on_4xx(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    ...
    assert mock_ocr_client.post.call_count == 1


async def test_extract_text_sends_correct_auth_and_file(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
    mock_ocr_client.post.return_value = _success_response()

    await extract_text(pdf, _make_config(api_key="secret-key-123"))

    call_kwargs = mock_ocr_client.post.call_args
...
@pytest.mark.parametrize("ext", [".pdf", ".png", ".jpg", ".jpeg", ".tiff", ".tif"])
async def test_extract_text_accepts_all_image_types(tmp_path, ext, mock_ocr_client):
    f = tmp_path / f"doc{ext}"
    f.write_bytes(b"fake-content")
    mock_ocr_client.post.return_value = _success_response()

    result = await extract_text(f, _make_config())

    assert result.text == "Hello world"


async def test_extract_text_exponential_backoff_delays(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")
...
async def test_extract_text_raises_on_non_json_response(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")

    bad_resp = mock.Mock(spec=httpx.Response)
    bad_resp.status_code = 200
    bad_resp.json.side_effect = ValueError("No JSON")
    bad_resp.text = "<html>Error page</html>"
    mock_ocr_client.post.return_value = bad_resp

    with pytest.raises(OCRError, match="non-JSON response"):
        await extract_text(pdf, _make_config())


async def test_extract_text_raises_on_malformed_api_response(tmp_path, mock_ocr_client):
    pdf = tmp_path / "doc.pdf"
    pdf.write_bytes(b"%PDF-fake")

    bad_resp = mock.Mock(spec=httpx.Response)
    bad_resp.status_code = 200
    bad_resp.json.return_value = {"error": "quota exceeded"}
    mock_ocr_client.post.return_value = bad_resp
